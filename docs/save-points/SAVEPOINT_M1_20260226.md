...
docs/iterations/M1_01_02252026_collector-instance/design.md
docs/iterations/M1_01_02252026_collector-instance/requirements.md
docs/iterations/M1_01_02252026_collector-instance/session-log.md
docs/iterations/M1_01_02252026_collector-instance/team-prompt.md
docs/iterations/M1_02a_02252026_interface-refactor/design.md
docs/iterations/M1_02a_02252026_interface-refactor/requirements.md
docs/iterations/M1_02a_02252026_interface-refactor/session-prompt.md
docs/iterations/M1_02b_02252026_replication-collectors/design.md
docs/iterations/M1_02b_02252026_replication-collectors/requirements.md
docs/iterations/M1_02b_02252026_replication-collectors/team-prompt.md
...
// Queried once by the orchestrator at the start of each collection cycle
// and passed to all collectors, avoiding redundant per-collector queries.
type InstanceContext struct {
    IsRecovery bool // true when instance is a standby (replica)
}
...
// CollectAll runs every registered collector, returns partial results on errors.
// ic provides per-cycle instance state queried once by the caller.
...
| **Total: 76** | | | **18 done, 8 deferred/skipped, 50 remaining** |
...
| # | Query | Gate | PG 14 | PG 15+ |
|---|-------|------|-------|--------|
| 1 | Q10 | is_in_backup | `SELECT pg_is_in_backup()` | Skip (function removed) |
| 2 | Q19 | pgss_info | Skip | `SELECT * FROM pg_stat_statements_info` |

### Version Gates Implemented (M1_02b)

| # | Query | Gate | PG 14 | PG 15 | PG 16+ |
|---|-------|------|-------|-------|--------|
| 3 | Q40 | replication_slots | Base columns + empty string placeholders | + `two_phase::text` | + `conflicting::text` |
...
| M1_02a | Interface refactor: add InstanceContext to Collector.Collect() signature | ✅ Done |
...
### What Was Just Completed (M1_02a + M1_02b)

**M1_02a — InstanceContext interface refactor (commit c50dbe1):**
Added `InstanceContext` struct to `collector.go` with `IsRecovery bool` field. Updated `Collector.Collect()` signature across all 8 existing collectors to accept `ic InstanceContext` as third parameter. `ServerInfoCollector` now reads `ic.IsRecovery` instead of querying `pg_is_in_recovery()` per cycle — eliminating one redundant DB roundtrip per scrape. Updated `Registry.CollectAll()` to accept and forward `InstanceContext`. All tests updated. 17 files changed.

**M1_02b — Replication collectors (commit aa76eee):**
Ported 5 PGAM queries (Q20, Q21, Q37, Q38, Q40) into 3 new collector files:
- `replication_lag.go`: byte + time lag per connected replica (primary only, 10s interval). 8 metrics per replica.
- `replication_slots.go`: WAL retention per slot (both roles, 60s). Version-gated SQL for PG 14 / 15 / 16+ using uniform 6-column scan regardless of version.
- `replication_status.go`: active replica count + per-replica connected metric (primary) or WAL receiver status + lag (replica). Role-dispatches via `ic.IsRecovery`.

All collectors use `queryContext()` for 5s timeout, `c.point()` for metric creation, `COALESCE` on all nullable SQL columns. 6 new files (3 collectors + 3 test files). Unit tests: 8 pass, 3 integration stubs skip cleanly.

Running total: 18/76 PGAM queries ported.

### What's Next (M1_03)

**M1_03 — Progress monitoring:** Port Q42–Q47 (vacuum/analyze/index build/cluster/basebackup/copy progress) plus checkpoint and bgwriter statistics. Requires version-gating for PG 17 (splits `pg_stat_bgwriter` → `pg_stat_checkpointer`). Also adds `pg_stat_io` collector (PG 16+).

Files to create: `progress.go` (vacuum/analyze/index/cluster/basebackup/copy progress), `checkpoint.go` (checkpoint + bgwriter + pg_stat_io), plus matching test files.
...
| 2026-02-26 | Omit init()/RegisterCollector pattern | Would require adding global registry; M1_01 uses explicit registration | Consistency with existing collectors; wire in main.go when needed |
| 2026-02-26 | Replication slots: 6-column uniform scan | Could scan conditionally per version | Single Scan() call regardless of PG version; empty string literals as placeholders for missing columns |
| 2026-02-26 | package collector (not collector_test) for gate tests | Gate var is unexported | Gate tests need access to unexported package-level var; coexists with collector_test files |
...
func (c *MyCollector) Collect(ctx context.Context, conn *pgx.Conn, ic InstanceContext) ([]MetricPoint, error) {
    // Role check — primary-only collectors (e.g. ReplicationLagCollector):
    // if ic.IsRecovery { return nil, nil }
    // Role-dispatch — role-aware collectors (e.g. ReplicationStatusCollector):
    // if ic.IsRecovery { return c.collectReplica(ctx, conn) }
...
- `pgpulse.replication.lag.total_bytes` (labels: `app_name`, `client_addr`)
- `pgpulse.replication.slot.retained_bytes` (labels: `slot_name`, `slot_type`)
...
3. Say: "Restoring from save point M1. Continue with M1_02a (InstanceContext refactor)."
