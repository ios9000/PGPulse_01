...
export function KeyMetricsRow({ currentMetrics }: KeyMetricsRowProps) {
  ...
  const active = m['pgpulse.connections.active']?.value ?? 0
  const maxConns = m['pgpulse.connections.max_connections']?.value ?? 0
  const connUtil = maxConns > 0 ? (active / maxConns) * 100 : 0
  const connStatus = thresholdColor(connUtil, 70, 90)

  const cacheHit = m['pgpulse.cache.hit_ratio']?.value
  const cacheStatus = cacheHit != null ? thresholdColor(cacheHit, 95, 90, true) : undefined

  const activeTxns = m['pgpulse.transactions.active']?.value ?? 0

  const replLag = m['pgpulse.replication.replay_lag_bytes']?.value
