...
func (s *APIServer) handleListDatabases(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range points {
		...
		switch p.Metric {
		case "pg.db.large_objects.count":
			agg.largeObjCount += int64(p.Value)
		case "pg.db.vacuum.dead_tuples":
			agg.deadTuples += int64(p.Value)
		case "pg.db.index.unused":
			agg.unusedIndexes += int64(p.Value)
		case "pg.db.table.bloat_ratio":
...
func (s *APIServer) handleGetDatabaseMetrics(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range points {
		...
		case"pg.db.index.unused":
			...
			im := getOrCreateIndex(indexMap, key, schema, table, index)
			im.Unused = true
		case"pg.db.index.unused_bytes":
			key := schema + "." + table + "." + index
			im := getOrCreateIndex(indexMap, key, schema, table, index)
			im.UnusedBytes = p.Value
		case"pg.db.index.bloat_ratio":
			key := schema + "." + table + "." + index
			im := getOrCreateIndex(indexMap, key, schema, table, index)
			im.BloatRatio = p.Value
		case"pg.db.index.wasted_bytes":
			key := schema + "." + table + "." + index
			im := getOrCreateIndex(indexMap, key, schema, table, index)
		...
		case"pg.db.vacuum.dead_pct":
			key := schema + "." + table
			vm := getOrCreateVacuum(vacMap, key, schema, table)
			vm.DeadPct = p.Value
		case"pg.db.vacuum.autovacuum_age_sec":
			key := schema + "." + table
			vm := getOrCreateVacuum(vacMap, key, schema, table)
			vm.AutovacuumAgeSec = p.Value
		case"pg.db.vacuum.autoanalyze_age_sec":
			key := schema + "." + table
			vm := getOrCreateVacuum(vacMap, key, schema, table)
			vm.AutoanalyzeAgeSec = p.Value

		// Schema sizes
		case"pg.db.schema.size_bytes":
		...
		// Sequences
		case"pg.db.sequence.last_value":
		...
		case"pg.db.function.total_time_ms":
			// Handled via calls entry — skip to avoid duplicates.

		// Catalogs
		case"pg.db.catalog.size_bytes":
		...
		case"pg.db.large_objects.size_bytes":
			resp.LargeObjectSizeBytes = p.Value
		case"pg.db.unlogged.count":
			resp.UnloggedCount = p.Value
		case"pg.db.partition.count":
