...
func TestPGRules(t *testing.T) {
	...
	tests := []struct {
		...
		{"conn_high positive", "rem_conn_high", MetricSnapshot{
			"pg.connections.active": 85, "pg.connections.max_connections": 100,
		}, "pg.connections.active", 85, true},
		{"conn_high negative", "rem_conn_high", MetricSnapshot{
			"pg.connections.active": 50, "pg.connections.max_connections": 100,
		}, "pg.connections.active", 50, false},
		{"conn_high boundary 80", "rem_conn_high", MetricSnapshot{
			"pg.connections.active": 80, "pg.connections.max_connections": 100,
		}, "pg.connections.active", 80, false}, // > 80, not >=
		{"conn_high missing max", "rem_conn_high", MetricSnapshot{
			"pg.connections.active": 85,
		}, "pg.connections.active", 85, false},

		// rem_conn_exhausted
		{"conn_exhausted positive", "rem_conn_exhausted", MetricSnapshot{
			"pg.connections.active": 99, "pg.connections.max_connections": 100,
		}, "pg.connections.active", 99, true},
		{"conn_exhausted negative", "rem_conn_exhausted", MetricSnapshot{
			"pg.connections.active": 90, "pg.connections.max_connections": 100,
		}, "pg.connections.active", 90, false},
		...
		{"commit_ratio positive", "rem_commit_ratio_low", MetricSnapshot{
			"pg.transactions.commit_ratio": 80,
		}, "pg.transactions.commit_ratio", 80, true},
		{"commit_ratio negative", "rem_commit_ratio_low", MetricSnapshot{
			"pg.transactions.commit_ratio": 95,
		}, "pg.transactions.commit_ratio", 95, false},

		// rem_repl_lag_bytes (10-100 MB)
		{"repl_lag positive", "rem_repl_lag_bytes", MetricSnapshot{
			"pg.replication.replay_lag_bytes": 20 * 1024 * 1024,
		}, "pg.replication.replay_lag_bytes", 20 * 1024 * 1024, true},
		{"repl_lag negative small", "rem_repl_lag_bytes", MetricSnapshot{
			"pg.replication.replay_lag_bytes": 5 * 1024 * 1024,
		}, "pg.replication.replay_lag_bytes", 5 * 1024 * 1024, false},
		{"repl_lag negative large", "rem_repl_lag_bytes", MetricSnapshot{
			"pg.replication.replay_lag_bytes": 200 * 1024 * 1024,
		}, "pg.replication.replay_lag_bytes", 200 * 1024 * 1024, false}, // >100 MB triggers critical instead

		// rem_repl_lag_critical (>100 MB)
		{"repl_lag_crit positive", "rem_repl_lag_critical", MetricSnapshot{
			"pg.replication.replay_lag_bytes": 200 * 1024 * 1024,
		}, "pg.replication.replay_lag_bytes", 200 * 1024 * 1024, true},
		{"repl_lag_crit negative", "rem_repl_lag_critical", MetricSnapshot{
			"pg.replication.replay_lag_bytes": 50 * 1024 * 1024,
		}, "pg.replication.replay_lag_bytes", 50 * 1024 * 1024, false},

		// rem_repl_slot_inactive
		{"slot_inactive positive", "rem_repl_slot_inactive", MetricSnapshot{
			"pg.replication.slot_inactive": 2,
		}, "pg.replication.slot_inactive", 2, true},
		{"slot_inactive negative", "rem_repl_slot_inactive", MetricSnapshot{
			"pg.replication.slot_inactive": 0,
		}, "pg.replication.slot_inactive", 0, false},

		// rem_long_txn_warn (60-300s)
		{"long_txn_warn positive", "rem_long_txn_warn", MetricSnapshot{
			"pg.transactions.oldest_active_sec": 120,
		}, "pg.transactions.oldest_active_sec", 120, true},
		{"long_txn_warn negative", "rem_long_txn_warn", MetricSnapshot{
			"pg.transactions.oldest_active_sec": 30,
		}, "pg.transactions.oldest_active_sec", 30, false},
		{"long_txn_warn too high", "rem_long_txn_warn", MetricSnapshot{
			"pg.transactions.oldest_active_sec": 400,
		}, "pg.transactions.oldest_active_sec", 400, false},

		// rem_long_txn_crit (>300s)
		{"long_txn_crit positive", "rem_long_txn_crit", MetricSnapshot{
			"pg.transactions.oldest_active_sec": 400,
		}, "pg.transactions.oldest_active_sec", 400, true},
		{"long_txn_crit negative", "rem_long_txn_crit", MetricSnapshot{
			"pg.transactions.oldest_active_sec": 200,
		}, "pg.transactions.oldest_active_sec", 200, false},
		...
		{"pgss_fill positive", "rem_pgss_fill", MetricSnapshot{
			"pg.statements.fill_pct": 97,
		}, "pg.statements.fill_pct", 97, true},
		{"pgss_fill negative", "rem_pgss_fill", MetricSnapshot{
			"pg.statements.fill_pct": 80,
		}, "pg.statements.fill_pct", 80, false},
