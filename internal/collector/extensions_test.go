...
func TestExtensionsCollector_WithPGSS(t *testing.T) {
	...
	installed := findMetric(points, "pg.extensions.pgss_installed")
	require.NotNil(t, installed)
	assert.Equal(t, 1.0, installed.Value)

	// Fill percentage must be present and in [0, 100]
	fillPct := findMetric(points, "pg.extensions.pgss_fill_pct")
	...
	statsReset := findMetric(points, "pg.extensions.pgss_stats_reset_unix")
...
func TestExtensionsCollector_WithoutPGSS(t *testing.T) {
	...
	assert.Nil(t, findMetric(points, "pg.extensions.pgss_fill_pct"),
		"pgss_fill_pct must not be emitted when pgss is not installed")
	assert.Nil(t, findMetric(points, "pg.extensions.pgss_stats_reset_unix"),
